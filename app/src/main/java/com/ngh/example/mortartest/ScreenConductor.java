...
public class ScreenConductor<S extends Blueprint> implements CanShowScreen<S>
{
    ...
    public void showScreen(S newScreen, S oldScreen, Flow.Direction direction)
    {
        ...
        MortarScope oldChildScope = null;
        if (oldChild != null)
        {
            oldChildScope = Mortar.getScope(oldChild.getContext());
        ...
        switchViews(oldChild, newChild);
        onScopeChange(oldChildScope, newChildScope);
    }

    protected void onScopeChange(MortarScope oldScope, MortarScope newScope)
    {
        // successors can override this for sending event
        // e.g. through Otto
    }

    protected void switchViews(View oldView, View newView)
    {
